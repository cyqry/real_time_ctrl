...
pub async fn execute(
    context: &Context,
    mut input_ctrl_cmd: InputCtrlCommand,
    ...
    let cmd = match input_ctrl_cmd.clone() {
        ...
        InputCtrlCommand::SetBigFile(file_path, target_path) => {
            ...
            let receiver = executor.submit_with_result(move || async move {
                loop {
                    match iter.next().await {
                        ...
                        Some(Ok((range, data))) => {
                            ...
                            match context_c.send_data_with_id(data_id_c.clone(), v.as_ref()).await {
                                Ok(_) => {}
                                Err(e) => return Some((ErrCode::ReadError, e)),
                            };
                        }
                        Some(Err(e)) => {
                            return Some((ErrCode::WriteError, anyhow!(e)));
                        }
                    }
                }
                None
    ...
    match context.agent.clone().write().await.req(&req_cmd).await? {
