...
// // Pin<Box<(dyn futures::Future<Output = ()> + std::marker::Send + '0)>> + std::marker::Send>>
// // Pin<Box<(dyn futures::Future<Output = ()> + std::marker::Send + 'static)>> + std::marker::Send + 'static)>>
// /// 异步执行器
// #[derive(Debug)]
...
//     fn send<'a>(
//         &'a self,
//         data: Option<Box<dyn FnOnce() -> Pin<Box<dyn Future<Output = ()> + Send + 'a>> + Send + 'static>>,
...
// }
//
// impl Sender for mpsc::Sender<Option<Box<dyn FnOnce() -> Pin<Box<dyn Future<Output=()> + Send>> + Send + 'static>>> {
//     fn send<'a>(&'a self, data: Option<Box<dyn FnOnce() -> Pin<Box<dyn Future<Output=()> + Send + 'a>> + Send + 'static>>) -> Pin<Box<dyn Future<Output=anyhow::Result<()>> + Send + 'a>> {
//         Box::pin(async {
