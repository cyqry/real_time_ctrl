...
- `Exec(String)` 是高危能力；Kik 与其他命令一样执行，不在客户端内保存授权策略或秘密。
...
- 命令执行使用 16 个有界并发许可；每个上传命令按服务端生成的数据 ID 使用容量为 2 的私有队列。
  命令与数据跨独立 TCP 流允许数据合法先到：预登记路由最多 16 条、合计 64 MiB、TTL 30 秒；命令
  到达后升级为活动路由。重复、超时或超过 64 条活动路由的数据必须拒绝，禁止无界自动分配。
- 下载类命令必须使用服务端在控制命令中保留的数据 ID，不能由 Kik 重新生成；这使服务端可在数据
  到达前完成会话、Kik 和方向绑定。
